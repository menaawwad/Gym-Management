...
public class addNewProductController {
	...
	void saveClick(ActionEvent event) {
		...
		if (!DBConnect.ifProductExist(barcodeTextField1.getText())) {
			Function.alert("Product all ready exist");
			flag = false;
		}
		...
		if (flag) {
			...
			Product product = new Product(name, barcode, costPrice, sellPrice, minPrice, weight, minOrder);
			DBConnect.AddNewProduct(product);
			Node node = (Node) event.getSource();
			Stage stage = (Stage) node.getScene().getWindow();
			stage.close();
		}
