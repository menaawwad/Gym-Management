...
public class editExpensesController implements Initializable {
	...
	public void getData(Expenses expenses) {
		ID = expenses.getID();
		txtName.setText(expenses.getName());
		datDate.setValue(expenses.getExpensesDate());
		datDate.setPromptText(expenses.getExpensesDate().toString());
		txtCost.setText(String.valueOf(expenses.getCost()));
		txtNote.setText(expenses.getNote());
	}
	...
	public void clickSave(ActionEvent event) {
		...
		if (flag) {
			Expenses expenses = new Expenses(ID, txtName.getText(), datDate.getValue(),
					Double.valueOf(txtCost.getText()),cboType.getValue(), txtNote.getText());
			DBConnect.updateExpenses(expenses);
			Node node = (Node) event.getSource();
			Stage stage = (Stage) node.getScene().getWindow();
			stage.close();
		}
